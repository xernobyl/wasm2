// Half-cube: 3 faces (6 triangles) as a fan; vertex position only.
// In the vertex shader we flip the cube-space position by the camera octant (sign of to_camera)
// so the stored 3 faces become the 3 visible faces — looks like a full cube.
...
// Flip cube-space pos so the back corner (1,1,1) points away from the camera (one of 8 octants).
