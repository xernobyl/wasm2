// Full warehouse scene ported from webgl2/webgpu reference.
// Reversed-Z depth output for mixing with rasterized geometry.
...
fn fs(vin: VertexOutput) -> FragmentOutput {
    ...
    if (u._pad1.x > 0.5) {
        out.color = vec4<f32>(0.15, 0.15, 0.2, 1.0);
        out.depth = 0.0;
        return out;
    }
    ...
    let clip_pos = u.view_projection * vec4<f32>(pos, 1.0);
    out.color = vec4<f32>(outColor, 1.0);
