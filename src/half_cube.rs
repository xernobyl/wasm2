...
impl HalfCube {
  pub fn new(gl: Rc<Gl>) -> Self {
    ...
    let index_buffer = gl.create_buffer().expect("Error creating ELEMENT_ARRAY_BUFFER.");
    ...
    let mvp_buffer = gl.create_buffer().expect("Error creating ELEMENT_ARRAY_BUFFER.");
  ...
  pub fn draw(&self) {
    ...
    self.gl.draw_elements_with_i32(Gl::TRIANGLE_FAN, 8, Gl::UNSIGNED_BYTE, 0);
  }

  pub fn draw_instanced(&self, count: i32) {
    self.gl.bind_vertex_array(Some(&self.vao));
    self.gl.draw_elements_instanced_with_i32(Gl::TRIANGLE_FAN, 8, Gl::UNSIGNED_BYTE, 0, count);
  }

  pub fn update_mvp(&self, matrices: &[f32]) {
    self.gl.bind_vertex_array(Some(&self.vao));
    self.gl.bind_buffer(Gl::ARRAY_BUFFER, Some(&self.mvp_buffer));
    self.gl.buffer_data_with_u8_array(Gl::ARRAY_BUFFER, as_u8_slice(matrices), Gl::DYNAMIC_DRAW);
  }

  pub fn update_positions(&self, positions: &[f32]) {
    self.gl.bind_vertex_array(Some(&self.vao));
    self.gl.bind_buffer(Gl::ARRAY_BUFFER, Some(&self.position_buffer));
    self.gl.buffer_data_with_u8_array(Gl::ARRAY_BUFFER, as_u8_slice(positions), Gl::DYNAMIC_DRAW);
  }
}
